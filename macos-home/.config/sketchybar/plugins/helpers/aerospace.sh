...
aerospace_focused_window() {
  echo "$(aerospace list-windows --focused | cut -d '|' -f 1 | grep -Eo '[0-9]+')"
}
...
aerospace_highlight_window_id() {
  ...
  if [ -n "$prev_window_id" ] && [ "$prev_window_id" -ne "$window_id" ]; then
...
remove_unmatched_items() {
 ...
 aerospace_highlight_focused_window() {
   local window_id=$(aerospace_focused_window)
   if [ -n "$window_id" ]; then
...
# remove default placeholder icon if we have actual windows
# in the space to supply us with icons
