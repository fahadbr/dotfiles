...
function! fzfc#lines(...)
  let [display_bufnames, lines] = fzfc#_lines(1)
...
" Colors
" ------------------------------------------------------------------
function! fzfc#colors(...)
  let colors = split(globpath(&rtp, "colors/*.vim"), "\n")
  if has('packages')
    let colors += split(globpath(&packpath, "pack/*/opt/*/colors/*.vim"), "\n")
  endif
  return s:fzf('colors', {
  \ 'source':  fzfc#_uniq(map(colors, "substitute(fnamemodify(v:val, ':t'), '\\..\\{-}$', '', '')")),
  \ 'sink':    'colo',
...
function! s:sort_buffers(...)
  let [b1, b2] = map(copy(a:000), 'get(g:fzfc#buffers, v:val, v:val)')
...
function! fzfc#ag(query, ...)
  ...
  return call('fzfc#ag_raw', insert(args, command, 0))
endfunction

" ag command suffix, [options]
function! fzfc#ag_raw(command_suffix, ...)
  if !executable('ag')
    return s:warn('ag is not found')
  endif
  return call('fzfc#grep', extend(['ag --nogroup --column --color '.a:command_suffix, 1], a:000))
endfunction

" command, with_column, [options]
function! fzfc#grep(grep_command, with_column, ...)
  let words = []
...
function! fzfc#buffer_tags(query, ...)
  let args = copy(a:000)
...
function! fzfc#helptags(...)
  ...
  let tags = exists('*uniq') ? uniq(sorted) : fzfc#_uniq(sorted)
...
" File types
" ------------------------------------------------------------------
function! fzfc#filetypes(...)
  return s:fzf('filetypes', {
  \ 'source':  fzfc#_uniq(sort(map(split(globpath(&rtp, 'syntax/*.vim'), '\n'),
...
command! -bar -bang Gcheckout call fzfc#gcheckout(<bang>0)

function! fzfc#gcheckout(...)
  let s:git_root = s:get_git_root()
  if empty(s:git_root)
    return s:warn('Not in git repository')
  endif

	let source = '(git tag -l ; git branch -a) | grep -v "*" | sed "s/remotes\/origin\///" | sed -E "s/^[[:space:]]+//" | sort | uniq'

  let command = 'GCheckout'
  let expect_keys = join(keys(get(g:, 'fzf_action', s:default_action)), ',')
  let options = {
  \ 'source':  source,
  \ 'sink*':   s:function('s:gcheckout_sink'),
  \ 'options': [
	\ '--ansi',
  \ '--inline-info',
	\ '--prompt', command.'> ',
  ...
  return s:fzf('gcheckout', options, a:000)
endfunction

function! s:gcheckout_sink(lines)
  if len(a:lines) < 2
    return
  endif
  ...
  let buf = bufnr('')
	if !empty(a:lines[1])
		...
		"execute 'Git' 'checkout' a:lines[1]
		let cmd = 'git checkout '.a:lines[1]
...
" ------------------------------------------------------------------
" fzfc#maps(mode, opts[with count and op])
" ------------------------------------------------------------------
...
" ----------------------------------------------------------------------------
" fzfc#complete - completion helper
" ----------------------------------------------------------------------------
